...
;(function($){
	$.fn.TMSlider=$.fn.TMS=$.fn._TMS=function(o){
			...
			img
				...
				.load(function(){
					setTimeout(function(){
						...
						if (_.itmslinks[_.show] != 'no-link') {	
							_.pic.attr('onclick', 'bp_redir(\'' + _.itmslinks[_.show] + '\')')
							_.pic.css('cursor', 'pointer')
						}
		...
		changeFu:function(n){
			...
			_.n=n
			_.next=_.itms[n]
			_.nextlink = _.itmslinks[n]
			_.direction=n-_.buff
			
			if (_.itmslinks[n] != 'no-link') {	
				_.pic.attr('onclick', 'bp_redir(\'' + _.itmslinks[_.show] + '\')')
				_.pic.css('cursor', 'pointer')
			} else {
				_.pic.removeAttr('onclick')
				_.pic.css('cursor', 'default')
			}
		...
		afterShow:function(){			
			...
			_.pic.attr('onclick', 'bp_redir(\'' + _.nextlink + '\')')
		...
		parseImgFu:function(){
			...
			$(_.items+' a',_.me)
				.each(function(i){
					//_.itmslinks[i]=$(this).attr('href')
					_.itmslinks[i]= (typeof($(this).attr('href')) != 'undefined')? $(this).attr('href') : 'no-link';
				})
