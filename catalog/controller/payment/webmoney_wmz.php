...
class ControllerPaymentWebmoneyWMZ extends Controller {
	protected function index() {
		...
		$this->data['LMI_PAYEE_PURSE'] = $this->config->get('webmoney_wmz_merch_z');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/webmoney_wmz.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/webmoney_wmz.tpl';
		} else {
			$this->template = 'default/template/payment/webmoney_wmz.tpl';
		}
	...
	public function success() {
		...
		$this->model_checkout_order->confirm($LMI_PAYMENT_NO, $this->config->get('webmoney_wmz_order_status_id'), 'Webmoney WMZ');
	...
	public function callback() {
		...
		$LMI_SECRET_KEY = $this->config->get('webmoney_wmz_secret_key');
		...
		if( $order_info['order_status_id'] != $this->config->get('webmoney_wmz_order_status_id')) {
			$this->model_checkout_order->update($LMI_PAYMENT_NO, $this->config->get('webmoney_wmz_order_status_id'),'Webmoney WMZ',TRUE);
		}
