...
class ControllerModuleLatest extends Controller {
	protected function index($setting) {
		...
		$this->data['quick_view_latest'] = $this->config->get('config_quick_view_latest');
		...
		foreach ($results as $result) {
			...
			if ($this->config->get('config_sticker_special_module_latest')) {
				if ((float)$result['special']) {
					if ($result['price'] > 0) {
			...
			if ($this->config->get('config_sticker_new_module_latest')) {
			...
			if ($this->config->get('config_sticker_popular_module_latest')) {
