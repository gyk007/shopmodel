...
class ControllerModuleQuickView extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {			
			...
			if ($this->config->get('config_sticker_special_quick_view')) {
			...
			if ($this->config->get('config_sticker_new_quick_view')) {
			...
			if ($this->config->get('config_sticker_popular_quick_view')) {
			...
			foreach ($results as $result) {
				...
				if ($this->config->get('config_sticker_special_quick_view_related')) {
					if ((float)$result['special']) {
						if ($result['price'] > 0) {
				...
				if ($this->config->get('config_sticker_new_quick_view_related')) {
				...
				if ($this->config->get('config_sticker_popular_quick_view_related')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/quick_view.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/module/quick_view.tpl';
			} else {
				$this->template = 'default/template/module/quick_view.tpl';
			}
