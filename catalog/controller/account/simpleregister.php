...
class ControllerAccountSimpleRegister extends Controller {
    ...
    public function index() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->data['simple_create_account'] && $this->validate()) {
            ...
            $data = array(
                ...
                'opt'               => $this->request->post['registration[custom_opt]']
            );
    ...
    private function validate() {
        ...
        $fax = $this->simple->get_value(Simple::SET_REGISTRATION, 'fax');       
        if (!empty($fax) && !$this->model_account_customer->getTotalAffiliatesByEmail($fax)) {
            $this->data['customer_fields']['main_fax']['error'] = 'такого партнера не зарегистрированно!!!';
            $error = true;
        }
