<?php 
class ControllerAccountQuickRegistration extends Controller {
	private $error = array();
  	...
  	public function index() {
    	...
    	$this->language->load('account/quick_registration');
		...
		$this->document->setTitle 
      	...
      	$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('account/quick_registration', '', 'SSL'),      	
        	'separator' => $this->language->get('text_separator')
      	);
		...
		if ($this->data['first_quick_registration_checkout']) {
		...
		if ($this->data['last_quick_registration_checkout']) {
		...
		if ($this->data['telephone_quick_registration_checkout']) {
		...
		if ($this->data['customer_groups_quick_registration_checkout']) {	
		...
		if ($this->data['address_1_quick_registration_checkout']) {		
		...
		if ($this->data['city_quick_registration_checkout']) {	
		...
		if ($this->data['post_code_quick_registration_checkout']) {
		...
		if ($this->data['country_quick_registration_checkout']) {
    	...
    	$this->data['action'] = $this->url->link('account/quick_registration', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/quick_registration.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/quick_registration.tpl';
		} else {
			$this->template = 'default/template/account/quick_registration.tpl';
		}
  	...
  	protected function validate() {		
		...
		if ($this->config->get('config_telephone_quick_registration_checkout')) {
		...
		if ($customer_group) {	
			if ($this->config->get('config_customer_groups_quick_registration_checkout')) {
				// Company ID
