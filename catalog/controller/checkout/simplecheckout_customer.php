...
class ControllerCheckoutSimpleCheckoutCustomer extends Controller {
    ...
    public function index() {
        ...
        $this->data['text_show_selector']                    = $this->language->get('text_show_selector');
        ...
        $this->data['simple_address_hide_if_logged'] = $this->customer->isLogged() ? $this->config->get('simple_address_hide_if_logged') : false;

        if ($this->data['simple_customer_hide_if_logged'] && !$this->data['simple_address_hide_if_logged']) {
            $this->data['simple_show_shipping_address'] = true;
            $this->data['simple_show_shipping_address_same_show'] = false;
            $this->data['shipping_address_same'] = false;
        } elseif (!$this->data['simple_customer_hide_if_logged'] && $this->data['simple_address_hide_if_logged']) {
            $this->data['simple_show_shipping_address'] = false;
            $this->data['simple_show_shipping_address_same_show'] = false;
            $this->data['shipping_address_same'] = true;
        }
    ...
    private function load_fields() {
        ...
        if ($this->customer->isLogged()) {
            ...
            if ($this->request->server['REQUEST_METHOD'] == 'GET') {
                ...
                $custom_checkout_customer = $this->parse_custom_data($main_checkout_customer);
                $custom_checkout_address = $this->parse_custom_data($main_checkout_address);
            }
        }
    ...
    private function parse_custom_data($data) {
        $result = array();
    ...
    public function update() {
        ...
        if ($address_id) {
            ...
            if (!empty($tmp_address) && is_array($tmp_address)) {
