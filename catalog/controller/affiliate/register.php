	...
	private $error = array();
  	...
  	public function index() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $code = $this->model_affiliate_affiliate->getCodeAffiliateByEmail($this->request->post['email']);
	  	 
              $this->redirect($this->url->link('affiliate/success', 'tracking='.$code['code'], 'SSL'));
    	} 
  	...
  	protected function validate() {
        ...
        if (!$this->model_affiliate_affiliate->getTotalAffiliatesByFax($this->request->post['fax'])) {
      		$this->error['warning'] = $this->language->get('error_faxexists');
    	}
        ...
        if (!$this->model_account_customer->getTotalCustomersByEmail($this->request->post['email'])) {
      		$this->error['warning'] = "E-mail в личном кабинете и в партнерсксом кабинете должны совпадать!!!! Зарегистрировать <a href = '/register' target='_blank'>ЛИЧНЫЙ КАБИНЕТ</a>";
    	}
