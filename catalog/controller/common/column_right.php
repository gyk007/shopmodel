...
class ControllerCommonColumnRight extends Controller {
	protected function index() {
		...
		if ($route == 'news/news_category' && isset($this->request->get['ncat'])) {
			$ncat = explode('_', (string)$this->request->get['ncat']);
				
			$layout_id = $this->model_catalog_news_category->getNewsCategoryLayoutId(end($ncat));			
		}
		
		if ($route == 'news/news' && isset($this->request->get['news_id'])) {
