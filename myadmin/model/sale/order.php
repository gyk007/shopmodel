...
class ModelSaleOrder extends Model {
	...
	public function getTotalSalesByMonth() {
      	$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > 0 and `date_added` > DATE_SUB(CURDATE(),INTERVAL 31 DAY)");

		return $query->row['total'];
	}
	public function getTotalSalesByDay() {
      	$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > 0 and `date_added` > DATE_SUB(CURDATE(),INTERVAL 1 DAY)");
	...
	public function getAbandonedOrderCount() {
		$sql = "SELECT COUNT(DISTINCT `email`) as total FROM `" . DB_PREFIX . "order` WHERE `order_status_id` = 0 AND `email` NOT IN (SELECT `email` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > 0 and `date_added` > DATE_SUB(CURDATE(),INTERVAL 31 DAY))";
	...
	public function getAbandonedOrderIds() {
		$sql = "SELECT GROUP_CONCAT(CAST(`order_id` AS CHAR)) as order_ids FROM `" . DB_PREFIX . "order` WHERE `order_status_id` = 0 AND `email` NOT IN (SELECT `email` FROM `" . DB_PREFIX . "order` WHERE `order_status_id` > 0 and `date_added` > DATE_SUB(CURDATE(),INTERVAL 31 DAY))";
