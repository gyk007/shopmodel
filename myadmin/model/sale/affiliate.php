...
class ModelSaleAffiliate extends Model {
	...
	public function deleteAffiliate($affiliate_id) {
        ...
        $mail = $this->getMailAffiliateById($affiliate_id);
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "affiliate SET fax = 'удален' WHERE fax = '" . $mail['email']. "' ");
        $this->db->query("UPDATE " . DB_PREFIX . "affiliate SET partner2 = 'удален' WHERE partner2 = '" . $mail['email']. "' ");
        $this->db->query("UPDATE " . DB_PREFIX . "affiliate SET partner3 = 'удален' WHERE partner3 = '" . $mail['email']. "' ");
        $this->db->query("UPDATE " . DB_PREFIX . "affiliate SET partner4 = 'удален' WHERE partner4 = '" . $mail['email']. "' ");
	}
    ...
    public function getDate($email) {
		$query = $this->db->query("SELECT date_added FROM " . DB_PREFIX . "affiliate WHERE LCASE(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
    ...
    public function getTupeAffiliate($affiliate_id) {
     ...
     public function updateActiv($email) {
		$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET approved = 0 WHERE email = '" . $email. "' ");
	
		 
	}
    public function getApprovedByEmail($email) {
		$query = $this->db->query("SELECT approved FROM " . DB_PREFIX . "affiliate WHERE LCASE(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
	
		return $query->row;
	}
    public function getIdAffiliateByEmail($email) {
		$query = $this->db->query("SELECT affiliate_id FROM " . DB_PREFIX . "affiliate WHERE LCASE(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
	
		return $query->row;
	}
	public function getAffiliateMainPartner($affiliate_id) {
		$query = $this->db->query("SELECT fax FROM " . DB_PREFIX . "affiliate WHERE affiliate_id = '" . (int)$affiliate_id . "'");
         return $query->row;
	}
	...
	public function getAffiliates($data = array()) {
		...
		if (isset($data['filter_approved']) && !is_null($data['filter_approved'])) {
			$implode[] = "a.tupe = '" . (int)$data['filter_approved'] . "'";
		}		
	...
	public function approve($affiliate_id) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET tupe = 1 WHERE affiliate_id = '" . (int)$affiliate_id . "'");
	...
	public function getTotalAffiliates($data = array()) {
		...
		if (isset($data['filter_approved']) && !is_null($data['filter_approved'])) {
			$implode[] = "tupe = '" . (int)$data['filter_approved'] . "'";
		}		
