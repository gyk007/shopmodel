		...
		if ($this->user->hasPermission('modify', 'localisation/currency')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$currency_id = $requestpart[1];				
					$result = $this->model_localisation_currency->getCurrency($currency_id);
					if($result[$column_name]){
						$this->model_localisation_currency->changeStatus($currency_id, $column_name, 0);
					} else {
						$this->model_localisation_currency->changeStatus($currency_id, $column_name, 1);
					}				
					$result = $this->model_localisation_currency->getCurrency($currency_id);
