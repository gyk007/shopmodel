		...
		if ($this->user->hasPermission('modify', 'localisation/language')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$language_id = $requestpart[1];				
					$result = $this->model_localisation_language->getLanguage($language_id);
					if($result[$column_name]){
						$this->model_localisation_language->changeStatus($language_id, $column_name, 0);
					} else {
						$this->model_localisation_language->changeStatus($language_id, $column_name, 1);
					}				
					$result = $this->model_localisation_language->getLanguage($language_id);
