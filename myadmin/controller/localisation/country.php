		...
		if ($this->user->hasPermission('modify', 'localisation/country')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$country_id = $requestpart[1];				
					$result = $this->model_localisation_country->getCountry($country_id);
					if($result[$column_name]){
						$this->model_localisation_country->changeStatus($country_id, $column_name, 0);
					} else {
						$this->model_localisation_country->changeStatus($country_id, $column_name, 1);
					}				
					$result = $this->model_localisation_country->getCountry($country_id);
