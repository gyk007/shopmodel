		...
		if ($this->user->hasPermission('modify', 'sale/coupon')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$coupon_id = $requestpart[1];				
					$result = $this->model_sale_coupon->getCoupon($coupon_id);
					if($result[$column_name]){
						$this->model_sale_coupon->changeStatus($coupon_id, $column_name, 0);
					} else {
						$this->model_sale_coupon->changeStatus($coupon_id, $column_name, 1);
					}				
					$result = $this->model_sale_coupon->getCoupon($coupon_id);
