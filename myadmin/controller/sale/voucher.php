	...
	private $error = array();
  	...
  	protected function getList() {
		...
		if ($this->config->get('config_buttons_apply') == 0) {
			...
			$this->data['text_send'] = $this->language->get('button_send');
		}
		...
		if ($this->user->hasPermission('modify', 'sale/voucher')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$voucher_id = $requestpart[1];				
					$result = $this->model_sale_voucher->getVoucher($voucher_id);
					if($result[$column_name]){
						$this->model_sale_voucher->changeStatus($voucher_id, $column_name, 0);
					} else {
						$this->model_sale_voucher->changeStatus($voucher_id, $column_name, 1);
					}				
					$result = $this->model_sale_voucher->getVoucher($voucher_id);
