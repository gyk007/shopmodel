...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['text_no_name'] = $this->language->get('text_no_name');
			...
			$this->data['all_orders_href'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&filter_customer=' . $order_info['firstname'] . ' ' . $order_info['lastname'], 'SSL');
			...
			foreach ($products as $product) {
				...
				$products_image = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product['product_id'] . "'");
				
				foreach ($products_image->rows as $product_image) { 
	...
	public function addCommission() {
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['affiliate_id']) {
				  $this->load->model('sale/affiliate');
 				 $mailPartner = $this->model_sale_affiliate->getAffiliateMainPartner($order_info['affiliate_id']);
                 $tupePartner = $this->model_sale_affiliate->getTupeAffiliate($order_info['affiliate_id']);                 
                $idPartner = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner['fax']);
              $mailPartner2 = $this->model_sale_affiliate->getAffiliateMainPartner($idPartner['affiliate_id']);
                 $idPartner2 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner2['fax']);
               $mailPartner3 = $this->model_sale_affiliate->getAffiliateMainPartner($idPartner2['affiliate_id']); 
               $idPartner3 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner3['fax']);
               $mailPartner4 = $this->model_sale_affiliate->getAffiliateMainPartner($idPartner3['affiliate_id']); 
               $idPartner4 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner4['fax']); 
              $mailAffiliate = $this->model_sale_affiliate->getMailAffiliateById($order_info['affiliate_id']);
 				 $affiliate_total = $this->model_sale_affiliate->getTotalTransactionsByOrderId($this->request->get['order_id']);
 			
				if (!$affiliate_total) {
				     
                     if($tupePartner[tupe] == 1){ 
                   ...
                   $this->model_sale_affiliate->addTransaction($idPartner['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 1 уровня ".$mailAffiliate['email'], 1, $this->request->get['order_id']);
                      $this->model_sale_affiliate->addTransaction($idPartner2['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 2 уровня ".$mailAffiliate['email'], 2, $this->request->get['order_id']);
                       $this->model_sale_affiliate->addTransaction($idPartner3['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 3 уровня ".$mailAffiliate['email'], 3, $this->request->get['order_id']);
                      $this->model_sale_affiliate->addTransaction($idPartner4['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 4 уровня ".$mailAffiliate['email'], 5, $this->request->get['order_id']);
                      }
                      if($tupePartner[tupe] == 0){ 
					 $this->model_sale_affiliate->addTransaction($order_info['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id'], $order_info['commission'], $this->request->get['order_id']);
                   $this->model_sale_affiliate->addTransaction($idPartner['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 1 уровня ".$mailAffiliate['email'], 1, $this->request->get['order_id']);
                      $this->model_sale_affiliate->addTransaction($idPartner2['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 2 уровня ".$mailAffiliate['email']."&nbsp;вы бы могли получить 2$ ", 1, $this->request->get['order_id']);
                       $this->model_sale_affiliate->addTransaction($idPartner3['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 3 уровня ".$mailAffiliate['email']."&nbsp;вы бы могли получить 3$ ", 1, $this->request->get['order_id']);
                      $this->model_sale_affiliate->addTransaction($idPartner4['affiliate_id'], $this->language->get('text_order_id') . $this->request->get['order_id']." от партнера 4 уровня ".$mailAffiliate['email']."&nbsp;вы бы могли получить 5$ ", 1, $this->request->get['order_id']);
                      }
    	...
    	public function addCommissionPartner() {
		$this->language->load('sale/order');
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			$this->load->model('sale/order');
			$mailPartner1 = $this->request->get['mailpartner'];
            $mymail = $this->request->get['mail'];
			$order_info = $this->model_sale_order->getOrder($this->request->get['order_id']);
			
			if (isset($order_info['affiliate_id'])) {
				$this->load->model('sale/affiliate');
				 
                 $idPartner1 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner1);
                 $mailPartner2 = $this->model_sale_affiliate->getAffiliateMainPartner($idPartner1['affiliate_id']);
                 $tupePartner = $this->model_sale_affiliate->getTupeAffiliate($order_info['affiliate_id']);   
                 $idPartner2 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner2['fax']);
                 $mailPartner3 = $this->model_sale_affiliate->getAffiliateMainPartner($idPartner2['affiliate_id']);
                 $idPartner3 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner3['fax']);
                 ...
                 $idPartner4 = $this->model_sale_affiliate->getIdAffiliateByEmail($mailPartner4['fax']);
                 $mailAffiliate = $this->model_sale_affiliate->getMailAffiliateById($order_info['affiliate_id']);
			     $affiliate_total = $this->model_sale_affiliate->getTotalTransactionsByOrderId($idPartner1);
				 
				if (!$affiliate_total) {
                     if($tupePartner[tupe] == 1){ 
					 $this->model_sale_affiliate->addTransaction($idPartner1['affiliate_id'],  'за партнера 1 уровня e-mail: '.$mymail  , 25 , $this->request->get['order_id']);
                     $this->model_sale_affiliate->addTransaction($idPartner2['affiliate_id'], 'за партнера 2 уровня e-mail: '.$mymail, 5, $this->request->get['order_id']);
                     $this->model_sale_affiliate->addTransaction($idPartner3['affiliate_id'], 'за партнера 3 уровня e-mail: '.$mymail, 10, $this->request->get['order_id']);
 					 $this->model_sale_affiliate->addTransaction($idPartner4['affiliate_id'], 'за партнера 4 уровня e-mail: '.$mymail, 30, $this->request->get['order_id']);
                     }
                   if($tupePartner[tupe] == 0){
                     $this->model_sale_affiliate->addTransaction($idPartner1['affiliate_id'],  'за партнера 1 уровня e-mail: '.$mymail, 25 , $this->request->get['order_id']);
                     $this->model_sale_affiliate->addTransaction($idPartner2['affiliate_id'], 'за партнера 2 уровня e-mail: '.$mymail."&nbsp;вы бы могли получить 5$ ", 0, $this->request->get['order_id']);
                     $this->model_sale_affiliate->addTransaction($idPartner3['affiliate_id'], 'за партнера 3 уровня e-mail: '.$mymail."&nbsp;вы бы могли получить 10$ ", 0, $this->request->get['order_id']);
 					 $this->model_sale_affiliate->addTransaction($idPartner4['affiliate_id'], 'за партнера 4 уровня e-mail: '.$mymail."&nbsp;вы бы могли получить 30$ ", 0, $this->request->get['order_id']);
					...
					$json['error'] = $affiliate_total; 
				}
			} else {
				$json['error'] = $order_info['affiliate_id'];
			}
		}
    ...
    public function removeCommissionPartner() {
		$this->language->load('sale/order');
