		...
		if ($this->user->hasPermission('modify', 'sale/customer')) {
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					$customer_id = $requestpart[1];				
					$result = $this->model_sale_customer->getCustomer($customer_id);
					if($result[$column_name]){
						$this->model_sale_customer->changeStatus($customer_id, $column_name, 0);
					} else {
						$this->model_sale_customer->changeStatus($customer_id, $column_name, 1);
					}				
					$result = $this->model_sale_customer->getCustomer($customer_id);
