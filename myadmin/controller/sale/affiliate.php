...
class ControllerSaleAffiliate extends Controller { 
	...
	public function approve() {
  	...
  	protected function getList() {
		...
		$countFive = $this->model_sale_affiliate->getCountAffiliate($result['email']);
        $lastyear  = mktime(0, 0, 0, date("m"),   date("d"),   date("Y")-1);
			...
			$this->data['affiliates'][] = array(
				...
				'approved'     => ($result['tupe'] ? $this->language->get('text_yes') : $this->language->get('text_no')),
				'date_added'   => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'selected'     => isset($this->request->post['selected']) && in_array($result['affiliate_id'], $this->request->post['selected']),
				'action'       => $action,
                'count'       => $this->model_sale_affiliate->getCountAffiliate($result['email'])
			);
		}	
