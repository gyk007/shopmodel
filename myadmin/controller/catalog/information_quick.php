<?php
class ControllerCatalogInformationQuick extends Controller { 
	private $error = array();
	...
	public function index() {
		...
		if (!$this->config->get('config_information_quick_edit')) {
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	private function getListing($information_id){
		...
		$this->data['information_title'] = $this->model_catalog_information->getInformationTitle($this->request->get['information_id']);
		...
		if ($this->user->hasPermission('modify', 'catalog/information_quick')) {
			if(isset($this->request->get['information_description'])){		
				if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
					$this->model_catalog_information->changeDescriptions((int)$information_id, $this->request->post);
				}
			}
			
			if(isset($this->request->get['information_data'])){		
				if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
					$this->model_catalog_information->changeInformationData((int)$information_id, $this->request->post);
				}
			}
		}
		...
		if(isset($this->request->get['information_description'])){		
			$this->template = 'catalog/quick_edit_information/quick_information_description.tpl';
		}
		
		if(isset($this->request->get['information_data'])){		
			$this->template = 'catalog/quick_edit_information/quick_information_data.tpl';
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = $this->url->link('catalog/information_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/information_quick/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		$this->data['link'] = $this->url->link('catalog/information_quick&token=', 'token=' . $this->session->data['token'] . $url, 'SSL');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/information_quick/update', 'token=' . $this->session->data['token'] . '&information_id=' . $result['information_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_title'] = $this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . '&sort=id.title' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . '&sort=i.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'catalog/quick_edit_information/quick_information_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['information_id'])) {
			$this->data['action'] = $this->url->link('catalog/information_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/information_quick/update', 'token=' . $this->session->data['token'] . '&information_id=' . $this->request->get['information_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/information_quick', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'catalog/quick_edit_information/quick_information_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function changeTitle() {
        ...
        $this->response->setOutput($this->model_catalog_information->changeTitle($id, $title, $language));
    }
	...
	public function changeSortOrder() {
        ...
        $this->response->setOutput($this->model_catalog_information->changeSortOrder($id, $sort_order));
    }
			...
			if(isset($this->request->get['object_id'])){
				...
				if(count($requestpart)==2){
					...
					if($result[$column_name]){
						$this->model_catalog_information->changeStatus($information_id, $column_name, 0);
					} else {
						$this->model_catalog_information->changeStatus($information_id, $column_name, 1);
					}				
					$result = $this->model_catalog_information->getInformation($information_id);
