<?php
class ControllerCatalogOptionQuick extends Controller {
	private $error = array();  
	...
	public function index() {
		...
		if (!$this->config->get('config_options_quick_edit')) {
			$this->redirect($this->url->link('catalog/option', 'token=' . $this->session->data['token'], 'SSL'));
		}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	private function getListing($option_id){
		$this->language->load('catalog/option');
		$this->load->model('catalog/option');
		...
		if ($this->user->hasPermission('modify', 'catalog/option_quick')) {
			if(isset($this->request->get['option_edit'])){		
				if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
					$this->model_catalog_option->editOption((int)$option_id, $this->request->post);
				}
			}
		}
		...
		if(isset($this->request->get['option_edit'])){		
			$this->template = 'catalog/quick_edit_option/quick_option_popup.tpl';
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['insert'] = $this->url->link('catalog/option_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/option_quick/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['link'] = $this->url->link('catalog/option_quick&token=', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		if(isset($this->request->get['option_edit'])){		
			return $this->getListing((int)$this->request->get['option_id']);
		}
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/option_quick/update', 'token=' . $this->session->data['token'] . '&option_id=' . $result['option_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . '&sort=od.name' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . '&sort=o.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'catalog/quick_edit_option/quick_option_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['option_id'])) {
			$this->data['action'] = $this->url->link('catalog/option_quick/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else { 
			$this->data['action'] = $this->url->link('catalog/option_quick/update', 'token=' . $this->session->data['token'] . '&option_id=' . $this->request->get['option_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/option_quick', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->template = 'catalog/quick_edit_option/quick_option_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
