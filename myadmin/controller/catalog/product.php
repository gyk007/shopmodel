...
// manufacturer
		...
		$categorieses = $this->model_catalog_category->getAllCategories();

		$this->data['categorieses'] = $this->getAllCategories($categorieses);
	...
	private function getAllCategories($categorieses, $parent_id = 0, $parent_name = '') {
		$output = array();

		if (array_key_exists($parent_id, $categorieses)) {
			if ($parent_name != '') {
				$parent_name .= $this->language->get('text_separator');
			}

			foreach ($categorieses[$parent_id] as $category) {
				$output[$category['category_id']] = array(
					'category_id' => $category['category_id'],
					'name'        => $parent_name . $category['name']
				);

				$output += $this->getAllCategories($categorieses, $category['category_id'], $parent_name . $category['name']);
			}
		}
