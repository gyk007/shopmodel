...
class ControllerCatalogProductQuick extends Controller {
	private $error = array(); 
  	...
  	public function index() {
		...
		if (!$this->config->get('config_product_quick_edit')) {
	...
	protected function getListing($product_id) {
		...
		if (isset($this->request->post['main_category_id'])) {
			...
			$this->data['main_category_id'] = $this->model_catalog_product_quick->getProductMainCategoryId($this->request->get['product_id']);
		} else {
  	...
  	protected function getList() {					
        ...
        if(isset($this->request->get['product_manufacturer'])){		
			return $this->getListing((int)$this->request->get['product_id']);
		}
		...
		foreach ($results as $result) {
			...
			$manufacturer = $this->model_catalog_product_quick->getProductManufacturer($result['product_id']);
        ...
        $this->data['sort_manufacturer'] = $this->url->link('catalog/product_quick', 'token=' . $this->session->data['token'] . '&sort=p2c.manufacturer' . $url, 'SSL');
