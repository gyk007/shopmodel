	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#' + id).replaceWith('<a id="' + id + '" onclick="removeBanIP(\'' + ip + '\');" class="button"><?php echo $text_remove_ban_ip; ?></a>');
			}
		}
	});	
}
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#' + id).replaceWith('<a id="' + id + '" onclick="addBanIP(\'' + ip + '\');" class="button"><?php echo $text_add_ban_ip; ?></a>');
			}
		}
	});	
};
