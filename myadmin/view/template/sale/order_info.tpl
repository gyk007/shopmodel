        ...
        <table class="form">
          ...
          <tr>
            ...
            <td><?php if ($invoice_no) { ?>
              <?php echo $invoice_no; ?>
              <?php } else { ?>
              <span id="invoice"><a id="invoice-generate" class="button"><?php echo $text_generate; ?></a></span>
              <?php } ?></td>
          </tr>
          <tr>
          ...
          <?php if ($fax) { ?>
          <tr>
            <td>e-mail главного партнера</td>
            <td><?php echo $fax; ?></td>
          </tr>
          <?php } ?>
          <tr>
          ...
          <tr style="background-color: #FF007B;">
            <td style="color: #FFF; ">Возногрождение при покупке партнерской пакета</td>
            <td>
              <?php if (!$commission_total) { ?>
              <span id="commissionpartner"><a id="commissionpartner-add" class="button"><?php echo $text_commission_add; ?></a></span>
              <?php } else { ?>
              <span id="commissionpartner"><a id="commissionpartner-remove" class="button"><?php echo $text_commission_remove; ?></a></span>
              <?php } ?></td>
          </tr>
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['success']) {
                ...
                $('#commissionpartner').html('<a id="commissionpartner-remove" class="button"><?php echo $text_commission_remove; ?></a>');
			}
		}
	});
});

$('#commissionpartner-add').live('click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/addcommissionpartner&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&mailpartner=<?php echo $fax; ?>&mail=<?php echo $email?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
			$('#commissionpartner').after('<img src="view/image/loading.gif" class="loading" style="padding-left: 5px;" />');			
		},
		complete: function() {
			$('.loading').remove();
		},			
...
$('#commissionpartner-remove').live('click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/removecommissionpartner&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		beforeSend: function() {
			$('#commissionpartner').after('<img src="view/image/loading.gif" class="loading" style="padding-left: 5px;" />');			
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#commissionpartner').html('<a id="commissionpartner-add" class="button"><?php echo $text_commission_add; ?></a>');
			}
		}
	});
});
